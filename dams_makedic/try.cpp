...
/* key を探し、該当するリストを返す
   リストが空の場合、 NULL が返る */
void PtrTry::Find(const char* key, PtrVect<void*>& result, char cond) const
{
  if (! *key) {
    // ここでマッチした
    if (_list != NULL) {
  ...
  for (int i = 0; i < count; i++) {
    if (_next->Element(i)->_letter == *key) {
      key++;
      _next->Element(i)->Find(key, list, cond);
...
void PtrTry::Remove(void* obj, char* key)
{
  ...
  for (int i = 0; i < count; i++) {
    if (_next->Element(i)->_letter == *key) {
      key++;
      _next->Element(i)->Remove(obj, key);
      return;
    }
  }
  return; // 該当するデータ無し
...
/* key を探し、該当するリストを返す
   リストが空の場合、 NULL が返る */
void StaticPtrTry::Find(const char* key, PtrVect<long>& result, char cond, long base)
{
  int i;
  StaticPtrTry* node;
  if (! *key) {
